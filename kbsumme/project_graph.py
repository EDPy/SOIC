...
def comparegraph():
    ...
    for obj in kbmeta_objects.all():
        pids.append(obj.pid)

    for pid in pids:
        series = pd.Series(sum_calculation(pid))
        pdprojects[projectName(pid)] = series
        pnames.append(projectName(pid))

    pdprojectsT = pdprojects.T
    ind = np.arange(len(pids))

    charttop = 0
    for index, row in pdprojects.iterrows():
        if pdprojectsT[index].sum() > 0:
            plt.bar(ind, pdprojectsT[index], 0.35, bottom=charttop, label=dict_Names[index])
            charttop += pdprojectsT[index]


    plt.ylabel('Cost')
    plt.title('Projects Cost Overview')
    plt.xticks(ind, pnames)
